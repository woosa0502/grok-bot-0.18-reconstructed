...
// Re-check B never started AFTER C completed (hole 3): a later start would still be a failure.
const bStartedMarkerAfterC = marker(B.id, `BSTART-${nonce}`);
const pass = aStartedMarker && !!aRunning && aCompletedNormally && aDoneMarker && bQueued
  && bAfterStop?.status === "stopped"   // exact terminal, not any TERMINAL (hole 2)
  && !bStartedMarker && !bStartedMarkerAfterC && cStartedMarker && cFinal?.status === "done";
const out = {
  case: "l18-queued-r6", at: new Date().toISOString(), nonce,
  ...
  evidence: {
    ...
    bId: B.id, bStatusBeforeCancel, bQueued, bStatusAfterCancel: bAfterStop?.status, bStartedMarker_MUST_BE_FALSE: bStartedMarker, bStartedMarkerAfterC_MUST_BE_FALSE: bStartedMarkerAfterC, aRunningObserved: !!aRunning, bAfterStopExactlyStopped: bAfterStop?.status === "stopped",
    cId: C.id, cStartedMarker, cFinalStatus: cFinal?.status,
  },
  verdict_pass: pass,
};
fs.writeFileSync(path.join(SP, "ev-l18-queued-r6.json"), JSON.stringify(out, null, 2));
