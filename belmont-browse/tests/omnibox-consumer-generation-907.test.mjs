...
const campaign = path.resolve(browse, "../data/artifacts/aside-full-restoration_20260907T225545Z");
const original = path.join(campaign, "reference-vm/components/AsideAgentManager/1.26.907.1712/assets");
const names = ["-page-C9UKSHnb.js", "search-view-Bczinz9d.js"];
const patcher = path.join(browse, "tools/patch-omnibox-generation.py");
const directory = mkdtempSync(path.join(tmpdir(), "aside-generation-907-"));
...
test("907 exact pinned outputs replay, parse and remain idempotent", () => {
  assert.deepEqual(JSON.parse(first.stdout).map((entry) => entry.after), [
    "3728e7e3ee176c54857016a938c485fdf5260bc059549f98d0a366f218d0f0a4",
    "6d19fc6c80208264869bdb01ab2f037b6f8a0acb469eefc52463ce64c1d45388",
  ]);
...
test("907 corrupt second asset fails before changing first asset", () => {
...
test("907 selections and equality distinguish identical results in different generations", () => {
...
test("907 commit rebinds an inherited selection to its new rendered generation", () => {
  ...
  evaluate(0, "lt", {
    ...
    en: () => true, tn: () => true, z() {}, ye: () => false, f: (value) => messages.push(value),
...
test("907 old keyboard HTTP closure rejects while current generation navigates", () => {
  ...
  const submit = (displayed) => evaluate(0, "He", {
    ...
    E: { current: false }, h: { current: true }, _: (value) => value.trim(), Wr: () => null,
    zr: (_, selected) => selected ?? { line: 0, state: "normal" }, Ze: (matches, line) => matches[line],
...
test("907 native keyboard handoff cannot borrow the new committed generation", () => {
  const calls = [];
  const submit = evaluate(0, "He", {
    t: "query", u: [match(10)], I: { current: normalize({ line: 0, resultSequenceId: 11 }) },
    L: { current: "query" }, b: { sessionId: "session" }, E: { current: false }, h: { current: true },
    _: (value) => value.trim(), Wr: () => null, zr: (_, selected) => selected,
...
test("907 pointer selection retains the event's rendered generation", () => {
  const calls = [];
  evaluate(1, "P", { v: (value) => calls.push(value) })(0, 10)();
  assert.equal(calls[0].resultSequenceId, 10);
});

test("907 scoped-query normalization and stale-query rejection remain present", () => {
  const raw = readFileSync(path.join(original, names[0]), "utf8");
  const guard = 'const O=tt(v.input??"",B.current),Z=(N.current&&jt(O,N.current))??O,G=_(Z),W=v.matches??[],We=j.current;';
  ...
  const result = evaluate(0, "r", {
    ...
    B: { current: "new query" }, _: (value) => value.trim(), j: { current: null },
