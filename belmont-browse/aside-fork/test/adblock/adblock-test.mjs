...
ws.on("open", async () => {
  try {
    ...
    await shot(internals.sessionId, "ui-shots/22-adblock-internals.png");
    ...
    out.testRuleExtended = {
      redirect: JSON.parse(await ev(internals.sessionId, testExpr("http://127.0.0.1:18777/redir.js", "script"))),
      important: JSON.parse(await ev(internals.sessionId, testExpr("http://127.0.0.1:18777/imp.js", "script"))),
      exception: JSON.parse(await ev(internals.sessionId, testExpr("http://127.0.0.1:18777/exc.js", "script"))),
      regex: JSON.parse(await ev(internals.sessionId, testExpr("http://127.0.0.1:18777/rx-77.js", "script"))),
    };
    // 3. real page: network + cosmetic + subframe + extended syntax
    const page = await openTab("http://127.0.0.1:18777/index.html");
    await sleep(6000);
    ...
    await shot(page.sessionId, "ui-shots/23-adblock-page.png");
    ...
    const generic = await openTab("http://127.0.0.2:18777/generic.html");
    await sleep(4000);
    ...
    const csp = await openTab("http://127.0.0.1:18777/csp.html");
    await sleep(3000);
    out.csp = JSON.parse(await ev(csp.sessionId, `JSON.stringify({inline: String(window.__inline), title: document.title})`));
    const cspCtrl = await openTab("http://127.0.0.1:18777/opener.html");
    await sleep(2000);
    ...
    await shot(blocked.sessionId, "ui-shots/24-adblock-blockpage.png");
    ...
    const page2 = await openTab("http://127.0.0.1:18777/index.html");
    await sleep(6000);
