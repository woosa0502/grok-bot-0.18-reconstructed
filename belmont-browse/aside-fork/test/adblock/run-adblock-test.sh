#!/bin/bash
# End-to-end ad-block check against the running fork (belmont-browse service, CDP 9333).
# usage: run-adblock-test.sh [result.json]
# Needs: the service up (see HANDOFF §1), node + ws in belmont-browse/node_modules.
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
OUT="${1:-$HERE/result-$(date +%Y-%m-%d).json}"; case "$OUT" in /*) ;; *) OUT="$PWD/$OUT";; esac
export SP="$HERE/../.."   # screenshots land in <aside-fork>/ui-shots/22..24
export PATH="/home/hoon/.nvm/versions/node/v26.8.1/bin:$PATH"
cd "$HERE/page"
python3 -m http.server 18777 --bind 127.0.0.1 >/dev/null 2>&1 & HP=$!
python3 -m http.server 18777 --bind 127.0.0.2 >/dev/null 2>&1 & HP2=$!
sleep 1
