...
bool TabSearchBubbleHost::ShowTabSearchBubble(
  ...
  // Aside fork: the original hosts the extension's tabsearch.html instead of
  // the WebUI tab search page.
  if (AsideTabSearchBubbleContentsView::MaybeShow(button_, profile_)) {
    return true;
  }
