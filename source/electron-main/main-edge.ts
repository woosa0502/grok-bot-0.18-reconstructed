...
import { decodeAttachmentBytesFromRpc } from "../shared/attachment-byte-transport.js";
...
function attachmentBytes(raw: unknown): Uint8Array {
  const bytes = decodeAttachmentBytesFromRpc(req(raw).bytesBase64);
  invariant(bytes != null, "stageAttachmentBytes requires canonical Base64 attachment bytes.");
  return bytes;
}
