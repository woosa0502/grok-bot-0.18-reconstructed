...
export function createCoordinatorInferenceRouter(options: {
  ...
  return {
    ...
    async dispatch(method: string, args: unknown): Promise<{ handled: boolean; value?: unknown }> {
      ...
      // Everything else falls through to the gateway/host runner. The host owns turns and
      ...
      // and JSON-transcript merge here were unreachable — the non-cursor sendPrompt/transcript
      // methods return at the top, and cursor routes to the host too — so they were removed to
      // keep a single authoritative owner rather than a dormant duplicate. (F-006)
      return { handled: false };
    },
  };
}
