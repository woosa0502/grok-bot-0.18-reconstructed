...
export interface SendMessageDependencies<Context> {
  ...
  /** Called after a delivery the model marked final: true; the runtime ends the turn once this step's checkpoint is persisted. */
  readonly onFinalDelivery?: () => void;
...
export function createSendMessageTool(deps: SendMessageDependencies<Context>) {
  const execute = async (
    ...
    const run = async (): Promise<SendMessageResult> => {
      ...
      if (input.final === true) deps.onFinalDelivery?.();
      return new SendMessageResult({
        result: {
          case: "success",
