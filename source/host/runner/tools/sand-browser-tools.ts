...
const BROWSER_TOOL_SPECS: readonly BrowserToolSpec[] = [
  ...
  { id: "BROWSER_SNAPSHOT", name: "browser_snapshot", op: "snapshot", description: "Capture a structured snapshot of the current page with [ref=eN] handles for interactive elements, including open shadow DOM and same-origin iframe content, with element states (checked/expanded/selected/required). This is the source of truth for page structure; refs are tied to the latest snapshot for that tab. Better than a screenshot for deciding what to click or type." },
