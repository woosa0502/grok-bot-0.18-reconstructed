...
export const INCREMENTAL_VACUUM_PAGES = 512;
export interface IndexEntry { readonly id: string; readonly kind: string; readonly timestampMs?: number; readonly role?: "user" | "assistant"; readonly content?: string; readonly text?: string; readonly toAgent?: { readonly kind?: string }; readonly message?: { readonly type?: string; readonly content?: string; readonly url?: string; readonly file_name?: string }; readonly file_path?: string; readonly file_name?: string; readonly width?: number; readonly height?: number }
...
function messageOf(entry: IndexEntry) { if (entry.kind === "message" && entry.toAgent != null && entry.toAgent.kind !== "agent") return null; const body = entryText(entry).trim(); return body.length === 0 ? null : { entryId: entry.id, role: entry.kind === "message" ? entry.role ?? "assistant" : "assistant" as const, timestampMs: wholeMs(entry.timestampMs), body: body.slice(0, INDEXED_BODY_MAX_CHARS) }; }
