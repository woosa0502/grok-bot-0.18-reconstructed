...
import { buildFtsMatchQuery, FTS_TOKENIZER, SNIPPET_CONTEXT_TOKENS } from "./content-search-query.js";
export { buildFtsMatchQuery, FTS_QUERY_MAX_TERMS, SNIPPET_CONTEXT_TOKENS } from "./content-search-query.js";
...
CREATE VIRTUAL TABLE IF NOT EXISTS messages_fts USING fts5(body, content='messages', content_rowid='id', tokenize='${FTS_TOKENIZER}', prefix='2 3');
...
CREATE VIRTUAL TABLE IF NOT EXISTS media_fts USING fts5(file_name, content='media', content_rowid='id', tokenize='${FTS_TOKENIZER}', prefix='2 3');
...
export function searchMessages(db: DatabaseSync, query: string, limit: number, excludedAgentIds: readonly string[] = []): MessageSearchResult[] {
  ...
  const result = rows(db, `SELECT
      ...
      SELECT fts_rowid, ts FROM (
        ...
        FROM (
          ...
          WHERE messages_fts MATCH ?
            AND m.agent_id NOT IN (SELECT value FROM json_each(?))
        )
      )
    ...
    WHERE messages_fts MATCH ?
    ORDER BY matched.ts DESC`, match, JSON.stringify(excludedAgentIds), limit, match);
