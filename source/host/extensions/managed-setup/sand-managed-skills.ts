...
import { readWorkflowScopeFields } from "../../../shared/workflow-scope.js";
...
export function fetchedManagedSkillToSandSkill(skill: FetchedManagedSkill): ManagedSkill | null {
  ...
  return { ...readWorkflowScopeFields(parsed), id, name, description: clampWorkflowDescription(parsed?.description != null && parsed.description.length > 0 ? parsed.description : skill.description), body };
}
