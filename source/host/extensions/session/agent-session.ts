...
export class SandAgentSessionStore {
  ...
  writeAgentProfileFile(agentId: string, profile: Partial<SandAgentProfile> & { name: string; description: string }): void {
    ...
    const runtime = profile.runtime ?? current?.runtime, browserJobProtocol = profile.browserJobProtocol ?? current?.browserJobProtocol;
    writeSandProfileFile(path, { name, description: profile.description.trim(), title: profile.title?.trim() ?? current?.title ?? "", avatarShape: profile.avatarShape?.trim() ?? current?.avatarShape ?? "", avatarColor: profile.avatarColor?.trim() ?? current?.avatarColor ?? "", ...(runtime == null ? {} : { runtime }), ...(browserJobProtocol == null ? {} : { browserJobProtocol }) });
  }
