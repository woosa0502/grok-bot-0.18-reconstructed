...
export function createSharedInstalledPluginsLoader(deps: SharedInstalledPluginsLoaderDeps): () => Promise<LoadedPlugins> {
  ...
  const resolveCurrentUserId = async (): Promise<number | null> => {
    try { const response = await dashboard.getMe(new GetMeRequest(), { timeoutMs: CURRENT_USER_RPC_TIMEOUT_MS }); return response.userId > 0 ? response.userId : null; }
  ...
  return async () => {
    ...
    if (result.sourceUnavailable) throw new Error("Could not refresh plugin skills because the marketplace is unavailable.");
