...
const ASIDE_BROWSE_DESCRIPTION = [
  ...
  "If it needs approval or an answer from the user it returns a message starting with [approval needed] or [question]; ask the user, then call Task again with resume=<its agent id>. Use the returned [aside-answer] envelope as the prompt, preserve its exact toolCallId, and put allow / deny / confirm / cancel / free text (or all question answers) in its answer field. Never reuse an envelope for a different question.",
