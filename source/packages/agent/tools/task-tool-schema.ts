...
export function buildTaskParametersSchema(configs: readonly TaskSchemaSubagentConfig[], options: TaskParametersSchemaOptions): TaskParametersSchemaResult {
  ...
  const fileAttachmentsField = z.array(z.string()).optional().describe("Optional array of file paths to read and attach to the subagent's context — use it to forward relevant media (e.g. images the user sent). Video files additionally need a video-capable subagent type; on builds without one, pass videos to no subagent and tell the user video analysis is unavailable.");
