...
| 22:30~23:22 | FAIL 13건 원인 분석·수정("다 개선해"). 포크 C++(aside:// 스킴 매핑, Import 응답 배열, 리눅스 가속기 5개 + IDC_ASIDE_*, 사이드바 메뉴 문구), 데몬 패치(WSL explorer.exe/xdg-open 폴더 열기, 907·909 재생성·재고정), 확장 자산 패치(Ctrl 표기·File Explorer 문구, 도구+테스트). 빌드 2회(379단계·3단계), 격리 창에서 실키로 재검 | 5묶음 전부 PASS(재검 행 15개). D(Dark 테마)와 E1(사이드바 체크)은 오판이었음, E2(MCP 스위치)는 원본 설계. 루트 테스트·브라우저 테스트(172) 통과. 23:22 사용자 Aside를 새 빌드(c3209e2f)로 복구 | `docs/atomic-sweep-summary-2026-09-10.md` 재검 절, `belmont-browse/docs/UPGRADE.md` |

## 지금 상태

- 사용자 Aside: daemon 129705 / Chrome 129742 (23:22 수정 빌드 c3209e2f로 재기동, GPU 켬; 그 전 112992/113027은 22:21, 그 전 76304/76363은 20:33 테스터 재기동, GALLIUM_DRIVER=d3d12 --ignore-gpu-blocklist
