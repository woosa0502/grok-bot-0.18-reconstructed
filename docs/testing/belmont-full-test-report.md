...
### 후속 10: 3차 외부 검토 6건 — 전부 확인·수정 (2026-08-31, 11차 갱신)

| # | 지적 | 판정·수정 |
|---|---|---|
| 1 | markPending 실패 무시(디스크 오류 시 조용히 휘발성 복귀) | **사실** — 실패 시 발신 ack에 경고 + telemetry persist_failed |
| 2 | "전사 중복 방지"는 보장 아님(전사 추가↔displayed 기록 사이 크래시 창) + 정산 전 크래시 시 워커 재실행 | **사실** — at-least-once 의미론과 두 크래시 창을 원장·§2에 정확 명시(원자화는 단일 트랜잭션 저장 필요, 유보) |
| 3 | 48h prune이 미전달 메시지·저장 결과까지 삭제(주말 꺼두면 유실) | **사실** — kind-인지 보존: payload 보유 마커(메시지·결과)는 14일, 감시형 wake만 48h. 동작 테스트 |
| 4 | grep 경계 잔여(헤드리밋 직전 다음 매치의 -B가 이전 매치 trailing으로 오염) + 문자열 테스트의 한계 | **사실** — 순수 함수(grep-projection.ts)로 추출, **줄번호 기반 귀속**으로 재작성, 검토자 사례 그대로 rg --json 이벤트 동작 테스트 3건 |
| 5 | 부트스트랩이 기존 봇 persona·드리프트된 스위퍼를 안 고침 | **사실** — updateAgent/updateAgentAutomation으로 재조정(진짜 멱등 복구) |
...
### 후속 12: 5차 외부 검토 — cloud-agent payload 소실 폐쇄 (2026-08-31, 13차 갱신)
검토자 판정(PROVISIONAL)이 정확했다: SendToAgent 유실 창은 닫혔지만 **cloud-agent 마커의 저장 결과는 rearm 재감시가 payload 없는 upsert로 덮어 소실 가능**했다(로컬 Codex 모드에선 CloudAgent 숨김이라 실경로 무영향 — 정확성 수정).

| 항목 | 수정 |
|---|---|
| cloud-agent 저장 결과 소실 | 2겹: ①**upsert payload 보존 불변식**(agentMessage/completion/taskPrompt는 clearOne으로만 제거) ②rearmCloudAgentWake가 저장 결과를 직접 재생(재감시 대신) |
...
### 후속 13: 추천 개선 묶음 (2026-08-31, 14차 갱신 — 사용자 지시 "추천 개선 묶음 진행해라")

| # | 항목 | 결과 |
|---|---|---|
| 1 | **봇별 도구 제한**(AUDIT-W1 완성) | 설정 agentToolPolicyByAgentId(denyTools; SendMessage는 저장·적용 모두 제외) + 툴셋 봇별 필터 + CreateAgent deny_tools. **라이브**: Scribe(deny External*·Computer) — 거부 도구 미노출, 자체 Shell 정상 |
| 2 | **크래시 창 dedupe** | 인바운드 전사 항목에 마커 id 스탬프 → 재전달 시 최근 80항목 대조로 중복 생략 — '전사 추가↔displayed 기록' 창 사실상 폐쇄 |
| 3 | **manager-only chat**(B-2 호스트 절반, 옵트인) | SAND_MANAGER_ONLY_CHAT=1 시 워커행 사용자 프롬프트를 턴 없이 거절+전사 공지(그룹·관리자 예외, 기본 꺼짐). **라이브**: 워커 2회 차단·공지 2건, 관리자 정상 |
| 4 | **그룹 턴 durable** | 게시는 원래 영속 — 휘발성이던 멤버 응답 턴에 마커(group=true), 턴 후 정산, rearm이 그룹 턴 재실행. **라이브**: 마커 생성→정산, 멤버 도달 |
| 5 | AUDIT-W6·F5 | 컴퓨터 유즈 감사 자식 귀속 · 기억 저장→재시작→회수→삭제 자동 E2E |

게이트: `npm run check` exit 0 — 테스트 **208/208**(신규 tests/recommended-bundle.test.mjs 7 + 기억 E2E 1). 이로써 잔여 PARTIAL은 AUDIT-B2 하나(승인 상태기계·UI 잠금 — 유보 사유 기록).

### 후속 14: memory synthesis(dreaming) 활성화 + 턴 종료 기억 기록 미배선 발견·수정 (2026-08-31, 15차 갱신 — 사용자 지시 "커밋 푸쉬하고 메모리 해라")

커밋·푸시(`3911afc`, 후속 13+검증 기록) 후 memory synthesis를 켜는 과정에서 **재구성 원본의 P1 결함을 라이브로 발견**했다.

| # | 항목 | 결과 |
|---|---|---|
| 1 | **AUDIT-W21 (P1, 신규 발견·수정)**: 턴 종료 기억 기록 경로 전체 미배선 | 러너 `#memoryStore`가 주입만 되고 안 읽힘 + 어댑터가 memoryStore/episodeProgress/isMemorableExchange 미전달 → settle scope에 기억 저장소 자체가 없어 **dreaming 증거·legacy 추출 모두 한 번도 안 돌았음**(회상은 별도 배선이라 정상 — 결함 은폐). 발견 방법: 호스트 SIGUSR1 인스펙터 + CDP 로그포인트 → `[TRACE] settle {hasStore:false}` 실측. 수정: 어댑터 3필드 통과 + 러너 지연 게터 배선 |
| 2 | **AUDIT-W22**: dreaming 로컬 활성화 3중 차단 해제 | ①게이트 pin이 Cursor 인증 bootstrap 영구 대기 → `pinGateWithLocalFallback`(로컬 즉시 pin) ②synthesis 실행기가 gemini-2.5-flash를 Pi에 그대로 요청("Unknown Pi Codex model") → `createSummarizationSession`(제공자 인지 — turn-run-shell:220 선례) ③런처가 `SAND_FEATURE_GATE_OVERRIDES=sand_memory_dreaming=1` 기본 주입(`mergeGateOverrides` — 명시값 우선, =0 옵트아웃) |
| 3 | **라이브 E2E (cycle45)** | 18:30:36 사실 프롬프트 → 18:30:45 응답 → **18:31:21 합성 기억 커밋**(profile.md "User's birthday is June 19. User cannot eat spicy food." + `.dreaming/synthesized` 출처 마커) → 다음 턴 회상 정답("생일은 6월 19일…"). 시작 시 temporal 검토도 가동(Belmont, 12.8초, no-work). 독립 재현: 제안→검증→적용 전체 9.9초 committed |
...
| AUDIT-5 | SendToAgent 휘발성 + marker만 영속 | **FIXED** (후속 8·9: 'agent-message' 영속 wake-kind — 송신 시 전문 영속·전달 후 정산·재시작 재전달, 크래시 라이브 검증. 의미론 at-least-once: 좁은 중복 창 존재 — 전사 추가↔displayed 기록 사이, wake 완료↔정산 사이 크래시 시 재전달/워커 재실행 가능. 영속 실패는 ack에 경고·telemetry 기록, payload 마커는 14일 보존) | (원 근거) agent-to-agent-messaging.ts:48 in-memory Map, 영속 wake-kind에 agent-message 없음 |
...
## 7. 수정 우선순위 (감사 제안 + 현 상태 — **2026-08-31 후속 9 기준 전부 종결/유보 분류**)

역사적 우선순위 목록(아래)은 전부 처리됐다. 현 상태: 1(커밋 완료) · 2=AUDIT-1 FIXED(후속 4) · 3=AUDIT-3 FIXED(후속 4) · 4=AUDIT-4 FIXED(후속 5→8→9 rev2, 자식 완전 재개만 재파견으로 대체) · 5=AUDIT-5 FIXED(후속 8·9, at-least-once — durable ledger의 goal/review 확장은 Phase B 유보) · 6=AUDIT-10 FIXED(후속 4·6) · 7=AUDIT-7/8 FIXED·9는 사용자 제외 지시 · 8=F1~F4 정리(후속 8). 잔여는 §0 요약의 유보/제외 목록이 유일한 진실원.

1. ~~미커밋 maxTokens·update_state 수정 검증 및 커밋~~
2. ~~production memory store·자동 추출·새 대화 recall 실제 연결 (AUDIT-1)~~
3. ~~실제 agent directory·manager routing 연결 (AUDIT-3)~~
4. ~~child settle/store/transcript 완전 분리 (AUDIT-4)~~
5. ~~durable task ledger + worker delivery/result/retry 구현 (AUDIT-5)~~ — goal/review 원장 확장은 Phase B
6. ~~Pi OAuth 경로·UI 상태·provider 진실원 통합 (AUDIT-10)~~
7. ~~WSL 미지원 routine/plugin/image 기능 숨기거나 로컬 구현 (AUDIT-7/8/9)~~
8. ~~false-green 테스트를 실제 production E2E로 교체 (AUDIT-F1/F3/F4)~~
