...
- **후속 10 (최신)**: 3차 외부 검토 6건 전부 사실 확인·수정 — 영속 실패 시 ack 경고+telemetry · at-least-once 의미론(두 크래시 창) 정밀 명시 · payload 마커 14일 보존(감시형만 48h) · grep 줄번호 귀속(grep-projection.ts 순수 함수 + rg 이벤트 동작 테스트) · 부트스트랩이 persona/스위퍼 드리프트 재조정 · 문서 정합(§2/§7). 게이트 196/196.
